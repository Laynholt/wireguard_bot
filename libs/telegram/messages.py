...
USER_HELLO = (
    ...
    f"📄 Используйте <code>/{BotCommands.REQUEST_NEW_CONFIG}</code>, чтобы получить новый конфигурационный файл.\n\n"
    ...
    f"📋 Используйте <code>/{BotCommands.MENU}</code> для просмотра доступных команд\n"
    f"ℹ️ Или <code>/{BotCommands.HELP}</code> для справки."
)
