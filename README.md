...
```json
{
    ...
    "telegram_system_monitor_enabled": true,
    "telegram_system_monitor_interval_seconds": 60,
    "telegram_system_monitor_cpu_threshold_percent": 90.0,
    "telegram_system_monitor_cpu_duration_minutes": 3,
    "telegram_system_monitor_ram_threshold_percent": 90.0,
    "telegram_system_monitor_ram_duration_minutes": 3,
...
* **`dns_server_name`** — имя DNS-сервера или его IP-адрес, который будет попадать в конфиги клиентов
  (например, имя контейнера с AdGuard Home: `"adguardhome"`. Можно указать несколько IP-адресов через пробел).
...
* **`is_dns_server_in_docker`** — флаг, находится ли DNS-сервер в Docker-сети:
  ...
  * `0` — внешний/хостовый DNS-сервер.
  (Используется, для определения локального IP-адреса DNS-сервера в Docker-сети).
---
